...
const Home = () => {
  const { allStudents, pagination, globalLoading } = useContext(StudentContext);
  const { user } = useContext(AuthContext);
  const [showModal, setShowModal] = useState(false);
  const navigate = useNavigate();

  // Use allStudents for accurate stats — never goes stale on refresh
  ...
  return (
    <div className="dashboard">
      ...
      <div className="dash-stats">
        {stats.map((s) => (
          <div
            className={`dash-stat-card ${globalLoading ? "stat-skeleton" : ""}`}
            key={s.label}
          ...
          >
            ...
            <div className="dsc-body">
              <div className="dsc-value">
                {globalLoading ? (
                  <span className="skeleton-text" style={{ width: 48 }} />
                ) : (
                  s.value
                )}
              </div>
